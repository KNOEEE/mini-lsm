...
private:
  ...
  inline void ClearSavedValue() {
    if (saved_value_.capacity() > (1 << 20)) {
      std::string empty;
      // free memory
      swap(empty, saved_value_);
    } else {
      // clear the content, but the capacity is saved (allocated memory)
      saved_value_.clear();
    }
  }
...
inline bool DBIter::ParseKey(ParsedInternalKey* ikey) {
  ...
  while (bytes_until_read_sampling_ < bytes_read) {
    bytes_until_read_sampling_ += RandomCompactionPeriod();
    // TODO record sample, may cause compaction
  }
...
void DBIter::Next() {
  ...
  if (direction_ == kReverse) {
    ...
    // iter_ is pointing to current key. We can now safedly move to the next to
    ...
    do {
      ...
      if (ParseKey(&ikey) && ikey.sequence <= sequence_) {
        ...
        if (value_type == kTypeDeletion) {
          ...
          if (saved_value_.capacity() > raw_value.size() + (1 << 20)) {
            std::string empty;
            swap(empty, saved_value_);
          }
          SaveKey(ExtractUserKey(iter_->key()), &saved_key_);
